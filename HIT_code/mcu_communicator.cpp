...
public:
                       ...
                       frame_buffer_index_(0)
    {
        ...
        try
        {
            ...
            {
                ROS_WARN("Failed to open serial port: %s — node will run without MCU data", serial_port_.c_str());
            }
        }
        catch (const std::exception& e)
        {
            ROS_WARN("Serial exception during port setup: %s — node will run without MCU data", e.what());
            ROS_WARN("If MCU is not connected, this is expected. Plug in USB and restart.");
        }
...
private:
    ...
    void sendMotionCommand(uint8_t motion_mode)
    {
        ...
        try
        {
            if (!serial_.isOpen())
            {
                ROS_ERROR_THROTTLE(5, "Serial port is CLOSED! Port: %s. Cannot send motion command.", 
                         serial_port_.c_str());
                return;
            }
    ...
    void sendNavigationCommand(float vx, float vy, float z_angle)
    {
        ...
        try
        {
            if (!serial_.isOpen())
            {
                ROS_ERROR_THROTTLE(5, "Serial port is CLOSED! Cannot send navigation command.");
                return;
            }
    ...
    void receiveThread()
    {
        ...
        while (ros::ok())
        {
            if (!serial_.isOpen())
            {
                ...
                {
                    // Silently retry — MCU may not be connected
                    ros::Duration(2.0).sleep();  // slow retry to avoid log spam
                    continue;
                }
            }
            ...
            catch (const std::exception& e)
            {
                ...
                try { if (serial_.isOpen()) serial_.close(); }
                catch (...) {}
            }
