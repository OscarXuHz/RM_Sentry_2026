...
# ---------- Launch file locations (no longer patched; kept for reference) ----------
_WORKSPACE_ROOT = Path(__file__).resolve().parent.parent.parent.parent  # AstarTraining/

# Destination for map PNGs consumed by the planner ($(find trajectory_generation)/map/)
_MAP_DEPLOY_DIR = (
    _WORKSPACE_ROOT / "HIT_code" / "sentry_planning_ws" / "src" / "sentry_planning"
    / "trajectory_generation" / "map"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Copy occ/bev/occtopo PNGs to $(find trajectory_generation)/map/ as occfinal/bevfinal/occtopo.png (default: on).",
    )
    parser.add_argument(
        "--no-deploy",
        action="store_false",
        dest="deploy",
        help="Do NOT copy map PNGs to the planner map directory.",
    )
...
def main() -> None:
    ...
    # Write machine-readable metadata so launch files can use the correct origin
    meta_path = out_dir / "map_meta.yaml"
    with open(meta_path, "w") as f:
        f.write("# Auto-generated by pcd_to_maps.py — do not edit manually\n")
        f.write(f"resolution: {args.resolution}\n")
        f.write(f"map_x_size: {args.map_x_size}\n")
        f.write(f"map_y_size: {args.map_y_size}\n")
        f.write(f"map_lower_x: {args.lower_x:.3f}\n")
        f.write(f"map_lower_y: {args.lower_y:.3f}\n")
        f.write(f"rows: {rows}\n")
        f.write(f"cols: {cols}\n")
    ...
    # --- Deploy map PNGs to planner directory ---
    if getattr(args, "deploy", True):
        _MAP_DEPLOY_DIR.mkdir(parents=True, exist_ok=True)
        shutil.copy2(str(out_dir / "occ.png"), str(_MAP_DEPLOY_DIR / "occfinal.png"))
        shutil.copy2(str(out_dir / "bev.png"), str(_MAP_DEPLOY_DIR / "bevfinal.png"))
        shutil.copy2(str(out_dir / "occtopo.png"), str(_MAP_DEPLOY_DIR / "occtopo.png"))
        shutil.copy2(str(meta_path), str(_MAP_DEPLOY_DIR / "map_meta.yaml"))
        print()
        print(f"Deployed maps to: {_MAP_DEPLOY_DIR}")
        print(f"  occfinal.png, bevfinal.png, occtopo.png, map_meta.yaml")

    # --- Launch files now read map_meta.yaml directly via $(eval) ---
    # No need to patch XML defaults; map_meta.yaml (deployed above) is the
    # single source of truth.
    print()
    print("Launch files read map_meta.yaml via $(eval) — no XML patching needed.")
    print(f"  map_meta.yaml deployed to: {_MAP_DEPLOY_DIR / 'map_meta.yaml'}")
