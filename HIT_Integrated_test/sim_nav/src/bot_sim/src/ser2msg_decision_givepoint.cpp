...
double g_line_speed = 0.0;
double g_angle_target = 0.0;
int g_xtl_flag = 0;
bool g_in_bridge = false;

void cmdVelCallback(const sentry_msgs::slaver_speed::ConstPtr &msg)
{
    g_line_speed = msg->line_speed;
    g_angle_target = msg->angle_target;
    g_xtl_flag = msg->xtl_flag;
    g_in_bridge = msg->in_bridge;

    cmd_vel.linear.x = g_line_speed;
    cmd_vel.linear.y = 0.0;
    cmd_vel.angular.z = g_angle_target;
    // printf("msg_received");
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub = nh.subscribe("/sentry_des_speed", 1000, cmdVelCallback);
