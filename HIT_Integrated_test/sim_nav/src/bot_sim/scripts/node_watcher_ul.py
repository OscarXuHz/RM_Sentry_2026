...
def check_all_nodes():
    ...
    if is_node_alive('map_server'):
        ...
        subprocess.Popen(['roslaunch', 'bot_sim', 'map_server_UL.launch', '--no-summary'])
        rospy.loginfo("restarting map_server_UL.launch")
        time.sleep(1)
...
subprocess.Popen(['rosbag', 'record', '/livox/imu_192_168_1_3','/3Dlidar', '--split', '--duration=10s', '--chunksize=10', '-O', '/home/sentry_train_test/recording/'+currentTime+'/'+currentTime])
